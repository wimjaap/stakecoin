...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("stakecoin"))
        return false;
...
{
    // Convert stakecoin:// to stakecoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("stakecoin://"))
    {
        uri.replace(0, 12, "stakecoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "StakeCoin.lnk";
}
...
{
    return GetAutostartDir() / "stakecoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("StakeCoin-Qt") + " " + tr("version") + " " +
    ...
    setWindowTitle(tr("StakeCoin-Qt"));
