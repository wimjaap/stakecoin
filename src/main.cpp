...
const string strMessageMagic = "StakeCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    int64_t nSubsidy = 0;
    if(nHeight == 1) {
        nSubsidy = 1500000 * COIN; //1.5 million STK premine
	}
	 else {
...
{
    int64_t nSubsidy;
    if (IsNewStakeReward(pindexPrev->nTime))
	nSubsidy = COIN * 1 / 2;
    else
...
{
    RenameThread("stakecoin-loadblk");
